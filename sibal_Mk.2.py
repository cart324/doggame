...
async def draw(ctx):
    ...
    if not is_started:  # 게임이 아직 시작하지 않은 경우
        ...
        turn = "select"  # 다음 턴 설정
        while len(drew_card) != 3:  # 카드 3장 드로우
...
async def help(ctx):
    await ctx.respond(
        "**게임 시작 전**\n"
        "> `/join`을 사용하여 게임 참여, `/leave`를 사용하여 게임 참여 취소\n"
        "> 모두 참여하면 `/start`를 사용하여 게임 시작, 누군가 정해진 스코어에 도달하면 게임이 끝납니다.\n"
        "> 참여한 인원은 `/user_list`를 사용하면 볼 수 있습니다.\n\n"
        "**게임 시작 후**\n"
        "> 먼저 술래가 `/draw`를 사용하여 카드 3장을 뽑아야 합니다.\n"
        "> 이후 `/select 번호`를 사용하여 원하는 카드를 한장 고릅니다.\n"
        "> 카드가 공개가 되면 나머지 참여자는 `/submit 내용`을 이용하여 답안을 제출합니다.\n"
        "> 이때 빈칸이 여러개라면 second, third까지 추가한 뒤 입력하면 됩니다.\n"
        "> 자신이 대답하였는지 확인하고 싶으면 `/check`를 사용하세요."
        "> 모두가 답안을 제출하면 술래가 `/choice 번호`를 사용하여 우승자를 고릅니다.\n\n"
        "**기타 기능**\n"
        "> 모든 사람은 자신이 술래일 때 게임중 딱 한번씩 `/choice_all`을 사용할 수 있습니다.\n"
        "> 답안을 제출하지 않은 사람이 1명 있을 때 `/democracy`를 사용하여 과반수의 찬성을 받고 스코어를 강제로 1 올릴 수 있습니다."
    )
...
async def help_short(ctx):
    await ctx.respond(
        "3줄 요약 버전\n"
        "> 술래 지목된 사람은 `/draw`후 `/select 번호` 하면 됨\n",
        "> 카드가 제시되면 나머지 사람은 `/submit 내용` 하면 됨\n",
        "> 모두가 답을 제출했으면 술래가 `/choice 번호`해서 우승자를 고르면 됨"
    )
